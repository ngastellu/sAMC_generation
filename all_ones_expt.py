...
def cnn_output_to_image(out, datadims, Tsoftmax):
    ...
    np.save('reshaped_out.npy', out.detach().cpu().numpy())

    probs = F.softmax(out/Tsoftmax, dim=1)
    print(probs.shape)
    ...
    for i in range(nrows):
        row = probs[0,0,i,:]
        img[0,0,i,:] = torch.multinomial(row).squeeze()
...
checkpoint = torch.load('model-checkpoint-1.pt', map_location=device)
...
Tsoftmax = configs.softmax_temp

img_size = 20
input_ones = torch.ones(1,1,2048,2048).cuda()
...
with torch.no_grad():
    ...
    start = perf_counter()
    img = cnn_output_to_image(out, dataDims, Tsoftmax)
    end = perf_counter()
    print(f'Done! [{end - start} seconds]', flush=True)

    np.save('final_img.npy', img)